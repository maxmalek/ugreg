// hacked in shape to build without the rest of the chromium src -- max
// via https://src.chromium.org/viewvc/chrome/trunk/src/base/safe_numerics.h?revision=177264&pathrev=177264
...
inline bool isValidNumericCast(Source source)
{
  ...
  static_assert(SourceLimits::is_specialized, "argument_must_be_numeric");
  static_assert(SourceLimits::is_integer, "argument_must_be_integral");
  static_assert(DestLimits::is_specialized, "result_must_be_numeric");
  static_assert(DestLimits::is_integer, "result_must_be_integral");

  return detail::IsValidNumericCastImpl<
...
inline Dest checked_numeric_cast(Source source) {
  CHECK(isValidNumericCast<Dest>(source));
